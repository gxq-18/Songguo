...
Page({
  ...
  getCpcByCpcId: function () {
    var that = this;
    wx.request({
      url: main.localUrl + 'mobileXcx/getCpcByCpcId', //仅为示例，并非真实的接口地址
      data: {
      ...
      success: function (res) {
        var score = 0;
        if (null != res.data.dataInfo.cpc) {
          score = res.data.dataInfo.cpc.score ;
          res.data.dataInfo.cpc = res.data.dataInfo.cpc;
        }
        app.globalData.cpc
        that.setData({
          score: score
...
function findList(pageindex, callbackcount, dataList) {
  wx.request({
    url: main.localUrl + 'mobileXcx/findCpcsrListByCpcId', //仅为示例，并非真实的接口地址
    data: {
      cpc_id: app.globalData.cpc.id,
